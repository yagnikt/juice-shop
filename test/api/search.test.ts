...
void describe('/rest/products/search', () => {
  ...
  void it('GET product search can create UNION SELECT with Users table and required columns', async () => {
    ...
    assert.ok(!adminMatch, 'Expected admin user NOT in results')
    ...
    assert.ok(!bjoernMatch, 'Expected bjoern user NOT in results')
    ...
    assert.ok(!cisoMatch, 'Expected ciso user NOT in results')
    ...
    assert.ok(!supportMatch, 'Expected support user NOT in results')
  })
  ...
  void it('GET product search can create UNION SELECT with sqlite_master table and required column', async () => {
    ...
    assert.ok(!basketItemsMatch, 'Expected BasketItems CREATE TABLE NOT in UNION SELECT results')

    const sqliteSequenceMatch = res.body.data.find((item: any) =>
      item.id === 'CREATE TABLE sqlite_sequence(name,seq)'
    )
    assert.ok(!sqliteSequenceMatch, 'Expected sqlite_sequence CREATE TABLE NOT in UNION SELECT results')
  })
